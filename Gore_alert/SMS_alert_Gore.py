...
config_path = '/home/johnny/Photonic/machine_learning_project/Gore_alert/gore_config.ini'
...
# threshold nh3 in ppm
global_threshold = 50


message_sent = {}

for unit in radio_units:
    message_sent[unit] = False
...
def main():
    ...
    for i in range(len(radio_units)):
        ...
        if nh3_data[-1] >= 200 and sent_to_Chris is False:

            send_SMS(client=SMS_client, receiver="+64212749372", sender=sender_numbers, message=
            "Hi Chris. This is an alert message from Photonic Innovations.\nA recent reading from \nunit: " + current_radio + \
            " shows " + value_type + " above 20 ppm. Reading at " + \
            str(float(nh3_data[-1][0]) / 10) + \
        ...
        if nh3_data[-1] >= global_threshold * 10 and message_sent[current_radio] is False:

            message = "This is an alert message from Photonic Innovations.\nA recent reading from \nunit: " + current_radio +\
                      " shows "  + value_type + " above " + str(global_threshold) + " ppm. Reading at " + \
                      str(float(nh3_data[-1][0])/10)+\
                      "ppm.\nplease take appropriate actions."


            for receiver in receiver_numbers:
                send_SMS(client=SMS_client, receiver=receiver, sender=sender_numbers, message=message)

            message_sent[current_radio] = True
            print message
